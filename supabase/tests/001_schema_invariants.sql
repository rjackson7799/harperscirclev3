...
select is((select count(*)::int from hc.log_event_types), 19,
  'the event-type enumeration is seeded (1A''s seven + 1B–1D''s three + 2A''s eight + 4A M1''s signed_out — the APP-09b access-log half)');
