...
select is((select count(*)::int from hc.log_event_types), 23,
  'the event-type enumeration is seeded (1A''s seven + 1B–1D''s three + 2A''s eight + 4A''s three + 5A''s one: conflict_resolved + 6A M3''s one: proposal_rejected)');
