...
select is((select count(*)::int from hc.log_event_types), 13,
  'the event-type enumeration is seeded (1A''s seven + audience_changed, object_approved, object_shared + 2A''s invite_issued/accepted/revoked)');
