...
select is((select count(*)::int from hc.log_event_types), 16,
  'the event-type enumeration is seeded (1A''s seven + audience_changed, object_approved, object_shared + 2A''s three invite events + member_removed, task_unassigned, object_share_revoked)');
