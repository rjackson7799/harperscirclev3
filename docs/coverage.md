...
## 2B — app increment (TSD §5.5–§5.11 app halves, §1.7; PRD §4.1; ADR-0013 contracts, ADR-0014)

Layers gain `app` (vitest against the route/module seams and the live
local stack — the CI "Application tests" step) and `e2e` (the Playwright
§11.4-3 walkthrough — green at build verification, deliberately NOT a CI
gate in 2B; ADR-0014).
...
| APP-04 | Non-enumeration byte-identity: sign-in failure bytes identical for wrong-password vs no-account (no GoTrue text escapes); throttled response existence-independent; reset request identical for account and ghost; create-account identical status/Location/body for fresh vs exists with the verification mail requested in both branches — the fresh branch's Set-Cookie recorded as the one forced §5.5 deviation (ADR-0014 D3); the invite-variant address derives from the TOKEN server-side | §5.5; PRD §4.1.7 | app | 2B | green | tests/routes/sign-in.test.ts · tests/routes/create-account.test.ts · tests/routes/reset.test.ts |
| APP-05 | Wasnt-me app half (ADR-0013 F3 contract): GET renders and touches nothing; destruction ONLY on the explicit POST — execute → IMMEDIATE kill (DB session revocation + rotation) → complete, in that order; a GoTrue outage leaves the durable pending row for the key-gated hc_pipeline sweep (/api/worker/security-actions); refusals one neutral shape | §5.11; ADR-0013 F3 | app | 2B | green | tests/routes/wasnt-me.test.ts |
| APP-06 | The founder door: Step N of 4 on exactly the four step screens (AC-AUTH-2); resume from durable state — no circle→1, circle+empty context→3, context→4 (AC-AUTH-9); step 2 writes THROUGH hc.create_circle with ADR-0011 local parts, closed situation list, per-subject zips (second defaults), declared slice; step-3 write guarded to the founder's own in-setup circle; completion promises pinned to Phase 1 only (AC-AUTH-5) with the §7.5 line saying the smaller true thing | PRD §4.1.3; AC-AUTH-2/5/9 | app | 2B | green | tests/setup/founder-door.test.ts · tests/hc/circle.test.ts |
...
| APP-09b | AC-AUTH-10, the access-log half: structurally unwritable in 2B — no sign-out event type, hc.log hc_internal-only, reserve spent. QUEUED as ADR-0014 finding 1 (owner bound amendment at round 10) | §5.5; ADR-0014 | app | **2B gate** | **pending** | ADR-0014 §findings |
| APP-10 | AC-PERM-3's app half (the §5.8 sessions row): remove_member → immediate DB session revocation (auth.sessions emptied, refresh revoked — no per-user admin logout exists on this GoTrue, probed); a refusal revokes nothing; a still-unexpired JWT in a LIVE second browser context reads NOTHING on its next request (RLS is the enforcement) | §5.8; §4.6.3 | app + e2e | 2B | green | tests/routes/account.test.ts · e2e walkthrough step 10 |
