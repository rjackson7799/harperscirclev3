...
| ID | Assertion | Source | Layer | Slice | Status | Test |
|---|---|---|---|---|---|---|
| RLY-01 | The outbox relay and sweeper scheduler as workers: pgmq delivery of drained outbox rows and sweeper re-queue lists; kill-before-transition and outbox-loss end-to-end (the A.5 worker-layer halves) | §4.2, §4.11, A.5 | worker | **2+** | **pending** | no worker runtime exists; the DB halves are green (FRZ-15, SWP-01) |
