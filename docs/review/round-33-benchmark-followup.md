...
Subsequent execution: a supported manual-entry fixture path was proven with claim constraints enabled and then used for 2,021 events/501 claimed tasks in a dedicated staging circle. The hosted HTTP run measured p95 **982 ms** over 150 valid responses. See `../ops/staging-home-performance.md` for the deployed commit, full method and limitations. This supersedes the absence of current HTTP timing above for that named workload only; database-only and mixed-workload gates remain open.
