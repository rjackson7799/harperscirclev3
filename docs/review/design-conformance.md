...
| 44px touch targets incl. × glyphs | §8 | §8.7 | min-heights across control classes; chip-dismiss padded; **6B close-out: `input[type='file']` joined the shared input rule** — it was the one visible input type never enumerated, so `/upload`'s control measured 253×21 at 390px until the R5/F-6 manifest legs audited that route for the first time. The CI half now pins the SELECTOR LIST, not just the declaration | touch-targets test · e2e audit at 390px |
