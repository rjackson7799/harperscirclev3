...
Updated September 7, 2026. This is the current execution order for the Round 33 handoff. Requirements, ADRs, coverage and the owed ledger retain their authority; this plan links them rather than replacing them. Routine work proceeds under standing owner authorization. Merge, production activation and new schema/authorization increments retain their existing gates.
...
| 5 | Implement the approved Admin foundation | Earn isolated auth lifecycle proof before schema changes; implement the bounded authorization and audit contract. | Owner approved at most two migrations and no deployment after c4a561c. Two draft migrations and the named audited-read adapter are implemented. Isolated run 34159091922 at 5573f03 passed 1,872 database tests, 34 application/factory tests and 19 actual-function MFA/concurrency assertions. Existing-state upgrade, ownership/default-permission corrections and database lint passed at 3196d1d (run 34163690732), together with the database regressions. Next: final review and remaining aggregate/browser gates. OW-36 blocks closure until the missing audited health-reader scope is resolved. See docs/review/slice-10-capability-checkpoint.md. Home gates are not waived. |
