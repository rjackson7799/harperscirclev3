...
| 5 | Plan the next product increment | Select the next requirement from the governing product plan and approve its scope/tier before implementation. | Owner authorized Admin foundation design and failing-test preparation ahead of Home disposition. Contract: docs/review/slice-10-authorization-contract.md. Schema/privilege implementation is a separate bundled decision; Home gates are not waived. |
