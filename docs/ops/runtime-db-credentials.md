...
| Hosted (Supabase) | At deploy, over the maintenance connection: `create role hc_runtime_login login password '<generated ≥32 random bytes>' in role hc_runtime;` then set `HC_DB_URL` to a pooler URL authenticating as that role and redeploy (the flip itself is the 4B B8 unit) | After the flip, over the new URL: `select rolsuper, rolbypassrls, rolcanlogin from pg_roles where rolname = current_user` must read `f · f · t`; `select array_agg(b.rolname order by 1) from pg_auth_members m join pg_roles b on b.oid = m.roleid where m.member = 'hc_runtime'::regrole` must read exactly `{anon, authenticated}`; a probe `select 1 from public.accounts` as the bare login (no SET ROLE) must be refused |
