...
The full database suite exposed an additional consequence of removing every direct metadata grant: the sweep-health operator read is also denied. Its safe view and scheduler still work, but the one-operation platform-stats scope does not include an audited alert reader. OPS-01 is therefore pending and OW-36 is OPEN. This is not accepted risk. Do not claim alert delivery or merge/activate Admin until a bounded correction or explicit owner disposition resolves it. The 032 test retains the internal view's data proof under its existing owner; it no longer presents that as proof of operator access.
