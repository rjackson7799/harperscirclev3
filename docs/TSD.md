...
**[ERRATUM, ADR-0043 D3 — the slice-8 close-out, RATIFIED 2026-09-03; the departures were named at 8B as plan Q4(1) and ADR-0041 Q-A, and this is the one line they were owed.]** The shipped query (`lib/hc/search#DOCUMENTS_SQL`) departs from the block above at **two points, both in the SELECT list, and nowhere else** — FROM, WHERE, ORDER and LIMIT are the text above verbatim, and the block is not rewritten. **(1)** `ts_headline` takes a **fourth argument**, the options string `StartSel=U+0002, StopSel=U+0003` (`HEADLINE_OPTIONS`), so a snippet arrives delimited by STX/ETX rather than `<b>`/`</b>`; `splitHeadline` turns it into parts in the module and React builds the `<mark>`, so a document's own `<b>` or `<script>` stays text and no surface file carries `dangerouslySetInnerHTML` (`tests/lint/search-surface-fence.test.ts` pins its absence, and the product tree's set of such sites, as an exact set). **(2)** The row's own **title** is selected as link text (`d.title`; a task's `t.title`; an event's `e.summary`). It discloses nothing the snippet does not: title is weight A of `tsv_summary`, so it is inside the text the vector was built from at every level. Tasks and timeline events are single-vector and add `, id` to the order; documents keep `order by rank desc` verbatim.
