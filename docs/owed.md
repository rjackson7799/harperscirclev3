...
| OW-27 | ADR-0043 D2 — slice 8's close-out, ruling ADR-0042 Q-F with ADR-0040 Q-A (round 30 with round 28) | MAJOR | **FRZ-13's freeze carve-out reaches a WRITE definer.** `hc.grant_vectors` (`20260815230009`) gives a coordinator who is not the objected-to member, under an **`unresolved`** freeze, `frozen = false` and `cap = 'view'`; `hc.visible_at` applies the cap as `least(…)`; and `view` is exactly `hc.claim_task`'s admission floor (`20260903120001`) — so **she can take a task while the circle is frozen.** Ruled a defect, not an allowance: the carve-out is read-only by intent on three independent sources (the migration's own header, FRZ-13's row, and the three sibling task-write definers each raising `freeze_active` from an explicit `state in ('open','unresolved')` test that never consults `hc.grant_vectors`). **Bounded to ONE function** — `complete_task`, `snooze_task` and `assign_task` are NOT exposed, because each raises `freeze_active` before it reaches its level test, which answers ADR-0042's own "adjacent and NOT verified" note in the safe direction | `hc.claim_task` carries the same explicit `state in ('open','unresolved')` test against `public.freezes` that its three siblings carry, landed in a **NAMED M-slot of slice 9's migration bound** (this row is what makes slice 9's plan gate read it), **and** a pgTAP case in `070_task_claim.sql` pins a carve-out coordinator REFUSED under an `unresolved` freeze while her READ through the carve-out still resolves — the control in the same file. Green only when both exist; `docs/coverage.md` **FRZ-17** is the row that carries the exposure until then | OPEN | Verified from the source at the close-out, not taken from the ADR: `20260815230009` (`unres_carved` → `cap = 'view'`; `least()` in `hc.visible_at`), `20260903120001`:115–117 (the `>= 'view'` floor, no freeze test of its own), `20260829120001`:252–257 and `20260829120002`:123–128, 202–207 (the three siblings' explicit guards). **Why no test caught it:** `070_task_claim.sql` opens its freeze with `insert into public.freezes (circle_id) values (…)` and `state` defaults to `'open'` (`20260815200005`:20), so 070:32–35 is an open freeze throughout and the `unresolved` path is exercised nowhere for this function. TSK-05's cell is marked accordingly and stays green on the pgTAP evidence it has. **Not `RISK(row)`** — an accepted risk is one nothing turns green, and this one is meant to be fixed. A fix is DDL and the close-out ships none |

**OPEN: 1 / 25.** TAKEN 2 · RISK 1 · CLOSED 17 · PROMOTED 6 · 27 rows. Re-tallied by
...
**AMENDED AT THE CLOSE-OUT (2026-09-03, ADR-0043 D2 — nothing above is
rewritten).** The plan gate's arithmetic, *"it opens 0 ledger rows and takes 7
out of `OPEN`"*, was true when written and is now one row out of date: the
close-out **opens OW-27**, ruling ADR-0042 Q-F. Re-derived with this file's own
parser at the close-out head: **27 rows · OPEN 1 / 25 · TAKEN 2 · RISK 1 ·
CLOSED 17 · PROMOTED 6**. **The burn-down quota is still MET, with one to
spare** — the rule is *close at least as many as you open, plus five*, and
slice 8 opens **1** while taking **7** out of `OPEN` (OW-26 `CLOSED(2f2c509)`
plus the six Q6 promotions); 7 ≥ 1 + 5. OW-27's home is slice 9, and the row is
what makes slice 9's plan gate read it. **OW-05 stays `TAKEN(8/Tier-3 pass)`** —
its quota is recurring and the leg backlog is not clear.
---
